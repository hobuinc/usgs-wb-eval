...
BUFFER=$2
rm buffered.*
CPL_DEBUG=ON ogr2ogr buffered.shp $LAYER.geojson  -sql "SELECT ST_Buffer(ST_Union(ST_Transform(a.geometry, 7587)), $BUFFER) from $LAYER a where rowid=0" -dialect "SQLITE"
...
rm polygon.*
CPL_DEBUG=ON ogr2ogr polygon.shp $LAYER.geojson \
    -sql "SELECT
